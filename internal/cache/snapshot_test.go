...
func TestFailureAfterPriorSuccessServesStaleCachedItems(t *testing.T) {
	store := NewStore("")
	store.UpsertSuccess("claude-code", "Claude", []model.QuotaItem{{ID: "x", Provider: "claude-code", Label: "X", Window: model.Window{ID: "session", Label: "S", Kind: "rolling"}, Unit: "percent", Limit: 100, Used: 10, Remaining: 90, PercentUsed: 10, Severity: "ok", Visible: true}}, time.UnixMilli(1000), 100, 200)
	store.MarkFailure("claude-code", "Claude", time.UnixMilli(1500), 3*time.Second, errors.New("rate limited"), 100)
	usage := store.Overview(time.UnixMilli(1500), []model.Provider{{ID: "claude-code", Label: "Claude", Source: "pull"}})
